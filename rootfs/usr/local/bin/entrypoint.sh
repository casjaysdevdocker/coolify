...
# @@Created          :  Tuesday, Sep 17, 2024 11:24 EDT
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for coolify
...
SERVICE_USER="coolify"  # execute command as another user
SERVICE_GROUP="coolify" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for coolify"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
